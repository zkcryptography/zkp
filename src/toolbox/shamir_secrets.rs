    ...
    pub fn complete<R: CryptoRng + RngCore>(secret: Scalar, threshold: usize, sparse_shares: &Vec<Option<Scalar>>, rng: &mut R) -> Result<SecretShare, String> {
        if threshold == 0 {
            ...
            return Ok(SecretShare {
                secret,
                shares: sparse_shares.iter().map(|s| match s { Some(x) => Some(*x), None => Some(secret)}).collect(),
                threshold,
            })
        }
        ...
        new_shares.sort();
        let new_shares = new_shares.iter().map(|n| n.y).collect();
        Ok(SecretShare {
            secret,
            shares: new_shares,
            threshold: threshold,
